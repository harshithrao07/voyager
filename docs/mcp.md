...
- Tools the server advertises **without a description** get one generated by the default Chat model during sync, so the resource catalog and its embeddings have meaningful text to match against. A provider-supplied description always wins and is never overwritten. See [AI Workflow Generator — What the assistant can use](ai-workflows.md#what-the-assistant-can-use).
